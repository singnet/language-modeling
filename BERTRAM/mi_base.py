...
async def ws(websocket, path):
    async for data in websocket:
        ...
        if data['event'] == 'mi' :
            try:
                mi, tokens, miout = getMI(data['sentence'])
                response = json.dumps({'event': 'success', 'mi': mi.tolist(), 'tokens': tokens, 'tokens_x': tokens, 'miout': miout, 'label_x': 'Position in sentence', 'label_y': 'Token weights'})
...
asyncio.get_event_loop().run_until_complete(
    websockets.serve(ws, '0.0.0.0', 8150))
asyncio.get_event_loop().run_forever()

### For local run
# if __name__ == "__main__":
#     sentence = "Mom writes to Dad with chalk on the board."
#     mi, tokens, miout = getMI(sentence)

#     print(offline.plot([go.Heatmap(z=mi, x=tokens, y=tokens)], show_link=True, link_text='Export to plot.ly', validate=True, output_type='file',
#                         include_plotlyjs=True, filename='plots/pt_norm_base.html', auto_open=False, image=None, image_filename='raw_base', image_width=800, image_height=600))