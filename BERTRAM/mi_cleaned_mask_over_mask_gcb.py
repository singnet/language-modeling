...
tokenizer = BertTokenizer(
    vocab_file='{}/data/vocab.txt'.format(base_path), do_lower_case=True)
    ...
    for i, token1 in enumerate(tokens):
        ...
        for j, token2 in enumerate(tokens):
            if i != j and token1 != token2:
                ...
                result[i].append(0)
    miout = ["{} {} {}".format(inv_vocab[tid1], inv_vocab[tid2], result[i][j]) for i, tid1 in enumerate(
        ids[0:tokens_length]) for j, tid2 in enumerate(ids[0:tokens_length])]
...
asyncio.get_event_loop().run_until_complete(
    websockets.serve(ws, '0.0.0.0', 8166))
