...
def getMI(sentence) :
    ...
    predicted_tokens = [predicted_token.item() for predicted_token in torch.argmax(scores, dim=1)]

    result = None
    tokens_with_predicted = tokens
    for i, token in enumerate(tokens) :
        if token == '[MASK]':
            token = inv_vocab[predicted_tokens[i]]
            tokens_with_predicted[i] = token
    ...
    return mi, tokens, tokens_with_predicted, miout
...
asyncio.get_event_loop().run_until_complete(
    websockets.serve(ws, '0.0.0.0', 8151))
asyncio.get_event_loop().run_forever()

### For local run
# if __name__ == "__main__":
#     sentence = "Mom writes to [MASK] with chalk [MASK] the board."
