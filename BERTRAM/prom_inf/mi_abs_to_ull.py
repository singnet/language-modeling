...
def getMI(tokens, tailnumber, filename):
    ...
    f = open("{}/data/ull/max/{}.ull".format(base_path, filename), "a")
    fu = open("{}/data/ull/modmax/{}.ull".format(base_path, filename), "a")
    fs = open("{}/data/ull/mod/{}.ull".format(base_path, filename), "a")
    ...
    [f.write("{} {} {} {} {}\n".format(i, tid1, j, tid2, max(result[i][j], result[j][i]))) for i, tid1 in enumerate(tokens[0:tokens_length]) for j, tid2 in enumerate(tokens[0:tokens_length]) if i != j]
    [fu.write("{} {} {} {} {}\n".format(i, tid1, j, tid2, max(math.fabs(result[i][j]), math.fabs(result[j][i])))) for i, tid1 in enumerate(tokens[0:tokens_length]) for j, tid2 in enumerate(tokens[0:tokens_length]) if i < j]
    [fs.write("{} {} {} {} {}\n".format(i, tid1, j, tid2, math.fabs(result[i][j])+math.fabs(result[j][i]))) for i, tid1 in enumerate(tokens[0:tokens_length]) for j, tid2 in enumerate(tokens[0:tokens_length]) if i < j]
