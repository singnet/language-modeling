...
def getMI(tokens, tailnumber, filename):
    ...
    f = open("data/ull/source/{}.ull".format(filename), "a")
    fu = open("data/ull/direct/{}.ull".format(filename), "a")
    fs = open("data/ull/merged/{}.ull".format(filename), "a")
        ...
        for j, token2 in enumerate(tokens):
            if i != j :
                if j > i :
                    ...
                    result[j][i] = scores[j][token1_id].item()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--filename', type=str, default="11-0", help='Target file name')

    args = parser.parse_args()

    lines = [line.rstrip('\n') for line in open('data/capital/{}.txt_headless_split_U'.format(args.filename)) if len(line) > 1]
