...
def report_on_stdin(args, cli_args):
  ...
  tokenizer = BertTokenizer.from_pretrained('examplebert-base-suarez/suarez_checkpoint')
  model = BertModel.from_pretrained('examplebert-base-suarez/suarez_checkpoint')
  LAYER_COUNT = 12
  FEATURE_COUNT = 768 
  ...
  # Load file and parse to lines

  # os.makedirs("path/to/directory", exist_ok=True)

  lines = [line.lower().rstrip('\n') for line in open('{}/data/subsets/{}'.format(base_path, cli_args.filename)) if len(line) > 1]

  filename = cli_args.filename

  os.makedirs("{}/data/ull/structural_probes_parses".format(base_path), exist_ok=True)

  f = open("{}/data/ull/structural_probes_parses/{}".format(base_path, filename), "w")
  f.close()

  f = open("{}/data/ull/structural_probes_parses/{}".format(base_path, filename), "a")

  for index, line in tqdm(enumerate(lines), desc='[demoing]'):
  # for index, line in tqdm(enumerate(["The fat ugly smelly cat ate my uncle"]), desc='[demoing]'):
    # Tokenize the sentence and create tensor inputs to BERT
    untokenized_sent = line.strip().split()

    f.write(" ".join(untokenized_sent))
    f.write("\n")
    ...
    with torch.no_grad():
      ...
      # [f.write("{} {} {} {} {}\n".format(0, "###LEFT-WALL###", i, t1, 0.0)) for i, t1 in enumerate(untokenized_sent[0:len(untokenized_sent)])]
      # [f.write("{} {} {} {} {}\n".format(i+1, t1, j+1, t2, distance_predictions[i][j])) for i, t1 in enumerate(untokenized_sent[0:len(untokenized_sent)]) for j, t2 in enumerate(untokenized_sent[0:len(untokenized_sent)]) if i < j]
      # f.write("\n")
      ...
      [f.write("{} {} {} {}\n".format(edges[0]+1, untokenized_sent[edges[0]], edges[1]+1, untokenized_sent[edges[1]])) for edges in predicted_edges]
      f.write("\n")
  f.close()
...
if __name__ == '__main__':
  argp = ArgumentParser()
  argp.add_argument('--experiment_config', type=str, default='example/demo-bert.yaml', required=False)
  ...
  # GC_LGEnglish_noQuotes_manual.txt
  # GC_LGEnglish_noQuotes_fullyParsed.txt
  ...
  report_on_stdin(yaml_args, cli_args)
